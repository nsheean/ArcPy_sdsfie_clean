...
"""
=====================================================================================
Script: Globally_Unique_Identifier_Select_fields_by_Alias_and_calculate_Field.py
...
Purpose:
  ...
  feature classes whose field ALIAS equals "Globally Unique Identifier". For each
  target, select NULL/placeholder values and populate GUIDs. Writes a QA/QC CSV.
...
Output:
  - GUID_Update_<timestamp>.csv in the project home.
=====================================================================================
"""
...
# ---------------- Configuration ----------------
TARGET_ALIAS = "Globally Unique Identifier"  # case-insensitive alias match
PLACEHOLDERS = {"", " ", "TBD", "NULL", "N/A", "NA", "NONE", "UNKNOWN"}  # for TEXT fields
...
CSV_PATH = os.path.join(PROJECT_HOME, f"GUID_Update_{TS}.csv")
...
def find_field_by_alias(dataset_path, alias_text):
    """Return (field_name, field_type, field_length) for the field with the given alias (case-insensitive)."""
    try:
        for f in arcpy.ListFields(dataset_path):
            if (f.aliasName or "").strip().lower() == alias_text.strip().lower():
...
def build_missing_query(temp_layer, field_name, field_type):
    ...
    if field_type == "GUID":
        # True GUID type: only NULL is meaningful
        return f"{fld} IS NULL"
    if field_type == "String":
        # Treat common placeholders as missing; be explicit about empty strings
...
def run():
    ...
    for cat_path, meta in datasets.items():
        ...
        # Resolve GUID field by alias
        guid_field, guid_type, guid_len = find_field_by_alias(cat_path, TARGET_ALIAS)
        if not guid_field:
            rows.append({
                "dataset": ds_name, "catalogPath": cat_path, "workspace": ws, "layers": lyr_names,
                "fieldName": "", "fieldType": "", "fieldLength": "", "editsMade": 0,
        ...
        tmp_lyr = arcpy.management.MakeFeatureLayer(cat_path, f"lyr_guid_{uuid.uuid4().hex[:8]}").getOutput(0)
        where = build_missing_query(tmp_lyr, guid_field, guid_type)
        if not where:
            rows.append({
                "dataset": ds_name, "catalogPath": cat_path, "workspace": ws, "layers": lyr_names,
                "fieldName": guid_field, "fieldType": guid_type, "fieldLength": guid_len or "",
                "editsMade": 0, "status": "Skipped", "reason": f"Unsupported field type: {guid_type}"
            })
        ...
        # Calculate GUIDs
        if guid_type == "GUID":
            status = calculate_guid_guidfield(tmp_lyr, guid_field)
        elif guid_type == "String":
            status = calculate_guid_textfield(tmp_lyr, guid_field, guid_len)
        else:
            status = f"Skipped - Unsupported field type: {guid_type}"
